...
#define _evaluateScriptWithPath hr_inno_exwp
...
#define JPDouble  hr_dou
#define JPFloat  hr_ft
typedef struct {double d;} JPDouble;
typedef struct {float f;} JPFloat;

#define fixSignature  hr_fffssnnaat
static NSMethodSignature *fixSignature(NSMethodSignature *signature)
{
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.09) {
        ...
        if (isReturnDouble || isReturnFloat) {
            BOOL dj =isReturnDouble;
            
            NSMutableString *types;
            if(dj){
                types = [NSMutableString stringWithFormat:@"%s@:",@encode(JPDouble)];
            }else{
                types = [NSMutableString stringWithFormat:@"%s@:",@encode(JPFloat)];
            }
//            types = [NSMutableString stringWithFormat:@"%s@:",JPFloat  ? @encode(JPDouble) : @encode(JPFloat)];
            int nn = signature.numberOfArguments;
            for (int i = 2; i < nn; i++) {
...
#define callSelector hr_cscscs

#define JPFix hr_pText
@implementation NSObject (JPFix)

#define JPFixedFlagKey hr_myPriKey
const static void *JPFixedFlagKey = &JPFixedFlagKey;

#define jp_methodSignatureForSelector hr_mfs1
#define jp_fixMethodSignature  hr_mfs2
+ (void)jp_fixMethodSignature
{
#if TARGET_OS_IPHONE
#ifdef __LP64__
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.1) {
        ...
        if (!flag.boolValue) {
            ...
            BOOL trDid = class_addMethod(self, originalSelector, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod));
            BOOL trDid1 = class_addMethod(self, originalSelector, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod));
            BOOL trDid2 = class_addMethod(self, originalSelector, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod));
            if (trDid2) {
...
#define _exceptionBlock  hr_myexb
static void (^_exceptionBlock)(NSString *log) = ^void(NSString *log) {
    NSCAssert(NO, log);
};
#define _JSLastCallStack hr_jlcsk
...
@implementation hr_JPE

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"

#pragma mark - APIS
+(void)hr_initcc_step1:(JSContext *)context{
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
#define _OC_formatJSToOC  hr_My_fffjso
        context[@"hr_My_fffjso"] = ^id(JSValue *obj) {
            return formatJSToOC(obj);
        };
        
#define _OChr_fmttOtS  hr_My_ffchfos
        context[@"hr_My_ffchfos"] = ^id(JSValue *obj) {
            return formatOCToJS([obj toObject]);
        };
#define hr_OC_getCProps  hr_O_gcp
        context[@"hr_O_gcp"] = ^id(JSValue *obj) {
            id realObj = formatJSToOC(obj);
            return objc_getAssociatedObject(realObj, kPropAssociatedObjectKey);
        };
        
#define hr_O_setCPs  hr_O_scp
        context[@"hr_O_scp"] = ^(JSValue *obj, JSValue *val) {
            id realObj = formatJSToOC(obj);
            objc_setAssociatedObject(realObj, kPropAssociatedObjectKey, val, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        };

#define _OC_superClsName  hr_oscnsn
        context[@"hr_oscnsn"] = ^(NSString *clsName) {
            Class cls = NSClassFromString(clsName);
            return NSStringFromClass([cls superclass]);
        };
        
#define autoConvertOCType  hr_aaacocccco
        context[@"hr_aaacocccco"] = ^(BOOL autoConvert) {
            _autoConvert = autoConvert;
        };
#define convertOCNumberToString  hr_acaactosoo
        context[@"hr_acaactosoo"] = ^(BOOL convertOCNumberToString) {
...
#define _OC_defineClass  hr_mydc
    context[@"hr_mydc"] = ^(NSString *classDeclaration, JSValue *instanceMethods, JSValue *classMethods) {
        return hr_dddCcs(classDeclaration, instanceMethods, classMethods);
    };
#define _OC_defineProtocol  hr_myPro
    context[@"hr_myPro"] = ^(NSString *protocolDeclaration, JSValue *instProtocol, JSValue *clsProtocol) {
        return hr_dPl(protocolDeclaration, instProtocol,clsProtocol);
    };
#define _OC_callI  hr_My_fun_c
    context[@"hr_My_fun_c"] = ^id(JSValue *obj, NSString *selectorName, JSValue *arguments, BOOL isSuper) {
        return callSelector(nil, selectorName, arguments, obj, isSuper);
    };
#define _OC_callC  hr_My_fun_1
    context[@"hr_My_fun_1"] = ^id(NSString *className, NSString *selectorName, JSValue *arguments) {
...
#define _OC_log hr_my_gol
    context[@"hr_my_gol"] = ^() {
        NSArray *args = [JSContext currentArguments];
        for (JSValue *jsVal in args) {
            id obj = formatJSToOC(jsVal);
            NSLog(@"JSE: %@", obj == _nilObj ? nil : (obj == _nullObj ? [NSNull null]: obj));
        }
    };
...
+ (void)startEngine
{
    ...
    [self hr_initcc:context];
    [self hr_initcc_step1:context];
//    [self hr_initcc_step2:context];
//    [self hr_initcc_step3:context];
    ...
    if(0){
        ...
        if (!path) {
            NSLog(@"test a bug");
            _exceptionBlock(@"can't find Jpe.tx");
        }
        runJs = [[NSString alloc] initWithData:[[NSFileManager defaultManager] contentsAtPath:path] encoding:NSUTF8StringEncoding];
        
        NSString* en = [runJs DESWithKey:@"My_jd_key" encryptOrDecrypt:kCCEncrypt];
        [en writeToFile:rjpath atomically:YES];
    }else{
        NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"run" ofType:@"dat"];
        NSString* desJs = [[NSString alloc] initWithData:[[NSFileManager defaultManager] contentsAtPath:path] encoding:NSUTF8StringEncoding];
        runJs = [desJs DESWithKey:@"My_jd_key" encryptOrDecrypt:kCCDecrypt];
        if([runJs length] == 0)
            return ;
    ...
    if ([_context respondsToSelector:@selector(evaluateScript:withSourceURL:)]) {
        [_context evaluateScript:runJs withSourceURL:[NSURL URLWithString:@"Jpe.tx"]];
    } else {
        [_context evaluateScript:runJs];
    }
}
...
+ (JSValue *)_evaluateScript:(NSString *)script withSourceURL:(NSURL *)resourceURL
{
    ...
    NSString* jsStr = [NSString stringWithFormat:@"%@ion(%@catc%@y_ctac(e.mes%@c%@);",@" ;(funct",@" ){try{\n%@\n}",@"h(e){hr_m",@"sage, e.sta",@"k)}})("];
    NSString *formatedScript = [NSString stringWithFormat:jsStr, [_regex stringByReplacingMatchesInString:script options:0 range:NSMakeRange(0, script.length) withTemplate:_replaceStr]];

//    NSString *formatedScript = [NSString stringWithFormat:@";(function(){try{\n%@\n}catch(e){hr_my_ctac(e.message, e.stack)}})();", [_regex stringByReplacingMatchesInString:script options:0 range:NSMakeRange(0, script.length) withTemplate:_replaceStr]];
    @try {
        if ([_context respondsToSelector:@selector(evaluateScript:withSourceURL:)]) {
...
static const void *hr_pKey(NSString *propName) {
...
static id hr_gPM(id slf, SEL selector, NSString *propName) {
    return objc_getAssociatedObject(slf, hr_pKey(propName));
}
static void hr_sPMP(id slf, SEL selector, id val, NSString *propName) {
    objc_setAssociatedObject(slf, hr_pKey(propName), val, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
//methodTypesInProtocol
static char *hr_mTypesInPl(NSString *protocolName, NSString *selectorName, BOOL isInstanceMethod, BOOL isRequired)
{
...
static void hr_dPl(NSString *protocolDeclaration, JSValue *instProtocol, JSValue *clsProtocol)
{
    const char *protocolName = [protocolDeclaration UTF8String];
    Protocol* newprotocol = objc_allocateProtocol(protocolName);
    if (newprotocol) {
        hr_aGMTPl(newprotocol, instProtocol, YES);
        hr_aGMTPl(newprotocol, clsProtocol, NO);
        objc_registerProtocol(newprotocol);
    }
}

static void hr_aGMTPl(Protocol* protocol,JSValue *groupMethods,BOOL isInstance)
{
    ...
    for (NSString *jpSelector in groupDic.allKeys) {
        ...
        NSString *selectorName = hr_cJ1Pelecting(jpSelector);
        ...
        if (typeEncode) {
            hr_adhodtocol(protocol, selectorName, typeEncode, isInstance);
            ...
            if (returnEncode.length > 0) {
                ...
                hr_adhodtocol(protocol, selectorName, encode, isInstance);
            }
        }
    }
}

static void hr_adhodtocol(Protocol* protocol, NSString *selectorName, NSString *typeencoding, BOOL isInstance)
{
...
static NSDictionary *hr_dddCcs(NSString *classDeclaration, JSValue *instanceMethods, JSValue *classMethods)
{
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        for (NSString *jsMethodName in methodDict.allKeys) {
            ...
            NSString *selectorName = hr_cJ1Pelecting(jsMethodName);
            ...
            if (class_respondsToSelector(currCls, NSSelectorFromString(selectorName))) {
                ...
                BOOL overrided = NO;
                for (NSString *protocolName in protocols) {
                    char *types = hr_mTypesInPl(protocolName, selectorName, isInstance, YES);
                    if (!types) types = hr_mTypesInPl(protocolName, selectorName, isInstance, NO);
                    if (types) {
    ...
    class_addMethod(cls, @selector(getProp:), (IMP)hr_gPM, "@@:@");
    class_addMethod(cls, @selector(setProp:forKey:), (IMP)hr_sPMP, "v@:@@");

    return @{@"cls": className, @"superCls": superClassName};
}

//getJSFunctionInObjectHierachy
static JSValue *hr_gSFInObrachy(id slf, NSString *selectorName)
{
    ...
    if (_currInvokeSuperClsName[selectorName]) {
        ...
        selectorName = [selectorName stringByReplacingOccurrencesOfString:@"_JP1S2U3P4E5R_" withString:@"_J22P"];
    }
...
static void hr_JPFdIcn(__unsafe_unretained id assignSlf, SEL selector, NSInvocation *invocation)
{
    ...
    NSString *JPSelectorName = [NSString stringWithFormat:@"_J22P%@", selectorName];
    JSValue *jsFunc = isBlock ? objc_getAssociatedObject(assignSlf, ass_JSValue)[@"cb"] : hr_gSFInObrachy(slf, JPSelectorName);
    if (!jsFunc) {
    ...
    for (NSUInteger i = isBlock ? 1 : 2; i < numberOfArguments; i++) {
        ...
        switch(argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '{': {
                NSString *typeString = hr_exSSce([NSString stringWithUTF8String:argumentType]);
    ...
    if (_currInvokeSuperClsName[selectorName]) {
        ...
        NSString *tmpSelectorName = [[selectorName stringByReplacingOccurrencesOfString:@"_JP1S2U3P4E5R_" withString:@"_J22P"] stringByReplacingOccurrencesOfString:@"SUPER_" withString:@"_J22P"];
    ...
    NSArray *params = hr_fmttOtSList(argList);
    char returnType[255];
    ...
    switch (returnType[0] == 'r' ? returnType[1] : returnType[0]) {
        #define JP_FWD_RET_CALL_JS \
            ...
            while (![jsval isNull] && ![jsval isUndefined] && [jsval hasProperty:@"__isPerformInOC"]) { \
                ...
                if ([jsval hasProperty:@"sel"]) {   \
                    ...
                    args = @[[_context[@"hr_fmttOtS"] callWithArguments:callRet ? @[callRet] : hr_fmttOtSList(@[_nilObj])]];  \
                }   \
                [_JSMethodForwardCallLock lock];    \
        ...
        case '{': {
            NSString *typeString = hr_exSSce([NSString stringWithUTF8String:returnType]);
            ...
            @synchronized (_context) {
                ...
                if (structDefine) {
                    ...
                    hr_gSDact(ret, dict, structDefine);
                    [invocation setReturnValue:ret];
                    free(ret);
                }
            }
...
static id callSelector(NSString *className, NSString *selectorName, JSValue *arguments, JSValue *instance, BOOL isSuper)
{
    ...
    if (inputArguments > numberOfArguments - 2) {
        ...
        id result = hr_ivkPtrModd(argumentsObj, methodSignature, sender, selector);
        ...
        switch (argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '{': {
                ...
                @synchronized (_context) {
                    ...
                    if (structDefine) {
                        size_t size = sizeOfStructTypes(structDefine[@"types"]);
                        void *ret = malloc(size);
                        hr_gSDact(ret, valObj, structDefine);
            ...
            default: {
                ...
                if ([(JSValue *)arguments[i-2] hasProperty:@"__isBlock"]) {
                    ...
                    Class JPBlockClass = NSClassFromString(My_JPBlock);
                    if (JPBlockClass && ![blkJSVal[@"blockObj"] isUndefined]) {
                        ...
                        __autoreleasing id cb = hr_gCCallkkb(arguments[i-2]);
                        [invocation setArgument:&cb atIndex:i];
                    }
                } else {
                    [invocation setArgument:&valObj atIndex:i];
...
static id hr_ivkPtrModd(NSMutableArray *origArgumentsList, NSMethodSignature *methodSignature, id sender, SEL selector) {
...
//block_methodSignatureForSelector
NSMethodSignature *hr_bk_mSeFStc(id self, SEL _cmd, SEL aSelector) {
...
#pragma mark -
//genCallbackBlock
static id hr_gCCallkkb(JSValue *jsVal)
{
    ...
    *signature = s;
    ...
    objc_setAssociatedObject(block, ass_JSValue, jsVal, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
if (!success) { class_replaceMethod(cls, selector, (IMP)func, method_getTypeEncoding(method));}}
        ...
        JP_HOOK_METHOD(@selector(methodSignatureForSelector:), hr_bk_mSeFStc);
        JP_HOOK_METHOD(@selector(forwardInvocation:), hr_JPFdIcn);
    });
...
//getStructDataWithDict
static void hr_gSDact(void *structData, NSDictionary *dict, NSDictionary *structDefine)
{
        ...
        switch(*structTypes) {
            ...
            case '{': {
                ...
                if (end != NSNotFound) {
                    ...
                    hr_gSDact(structData + position, subDict, subStructDefine);
                    position += size;
                    structTypes += end;
                    break;
                }
...
//extractStructName
static NSString *hr_exSSce(NSString *typeEncodeString)
{
...
//blockTypeIsObject
static BOOL hr_bTIsObjj(NSString *typeString)
{
    return [typeString rangeOfString:@"*"].location != NSNotFound || [typeString isEqualToString:@"id"];
}
//blockTypeIsScalarPointer
static BOOL hr_bTIrPr(NSString *typeString)
{
...
static NSString *hr_cJ1Pelecting(NSString *selectorString)
{
...
static id formatOCToJS(id obj)
{
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSDictionary class]] || [obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSDate class]]) {
        return _autoConvert ? obj: hr_wjbO([JPBoxing boxObj:obj]);
    }
...
static id hr_fmttOtSList(NSArray *list)
{
...
static NSDictionary *hr_wjbO(id obj)
{
    if (!obj || obj == _nilObj) {
        return @{@"__isNil": @(YES)};
    }
...
+ (id)formatOCToJS:(id)obj
{
    ...
    return [context[@"hr_fmttOtS"] callWithArguments:@[formatOCToJS(obj)]];
}
...
{
    return hr_gSDact(structData, dict, structDefine);
}
